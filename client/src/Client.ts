...
export abstract class ExternalHooks {
    ...
    public static OnSyncCurrentBid: (playerId: number, bidType: BidType, bidTrump: Card, bidValue: number) => void = () => { };
...
export function RequestBid(bidType: BidType, bidTrump: Card, bidValue: number, selectedEmoji: string) {
    if (!isOpen) { alert("you are not connected to the internet"); return; }
    ws.send(SerializeRequestBid(auth_id, auth_pw, bidType, bidTrump, bidValue,BidEmotion.neutral));
}
...
function onSyncCurrentBid(data: ArrayBuffer) {
    let { playerId, bidType, bidTrump, bidValue } = DeserializeSyncCurrentBid(data);
    ExternalHooks.OnSyncCurrentBid(playerId, bidType, bidTrump, bidValue);
}
