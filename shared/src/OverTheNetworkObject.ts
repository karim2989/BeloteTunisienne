...
//#region Bidding
export function SerializeRequestBid(id: number, pw: number, bidType: BidType, bidTrump: Card, bidValue: number, emotion: BidEmotion): OverTheNetworkObject {
    let output = new ArrayBuffer(1 + (6 * 4)) //1 byte + 6 elements * 4 bytes
    let dv  = new DataView(output)

    dv.setUint8(0,ObjectType.RequestBid)
    dv.setInt32(1,id)
    dv.setInt32(5,pw)
    dv.setInt32(9,bidType)
    dv.setInt32(13,bidTrump as number)
    dv.setInt32(17,bidValue)
    dv.setInt32(21,emotion)
    return output;
}

export function DeserializeRequestBid(input: OverTheNetworkObject): { id: number, pw: number, bidType: BidType, bidTrump: Card, bidValue: number, emotion: BidEmotion } {
    if (input.byteLength != 25) throw 'Corrupt request'
    let dv = new DataView(input);
    return {
        id: dv.getInt32(1),
        pw: dv.getInt32(5),
        bidType: dv.getInt32(9) as BidType,
        bidTrump: dv.getInt32(13) as Card,
        bidValue: dv.getInt32(17),
        emotion: dv.getInt32(21) as BidEmotion
    };
}
...
export function SerializeSyncCurrentBid(playerId: number, bidType: BidType, bidTrump: Card, bidValue: number, emotion: BidEmotion): OverTheNetworkObject {
    let output = new ArrayBuffer(1 + (5 * 4)); // 1 byte + 5 elements * 4 bytes
    let dv = new DataView(output);
    dv.setUint8(0,ObjectType.SyncCurrentBid);
    dv.setInt32(1,playerId);
    dv.setInt32(5,bidType);
    dv.setInt32(9,bidTrump as number);
    dv.setInt32(13,bidValue);
    dv.setInt32(17,emotion);
    return output;
}

export function DeserializeSyncCurrentBid(input: OverTheNetworkObject): { playerId: number, bidType: BidType, bidTrump: Card, bidValue: number, emotion: BidEmotion } {
    if (input.byteLength != 21) throw 'Corrupt request';
    let dv = new DataView(input);
    return {
        playerId: dv.getInt32(1),
        bidType: dv.getInt32(5) as BidType,
        bidTrump: dv.getInt32(9) as Card,
        bidValue: dv.getInt32(13),
        emotion: dv.getInt32(17) as BidEmotion
    };
}

export function SerializeSyncCurrentPlayer(playerId: number): OverTheNetworkObject {
    let output = Buffer.alloc(1 + 4); // 1 byte + 4 bytes
