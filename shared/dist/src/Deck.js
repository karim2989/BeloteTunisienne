...
export class DeckUtils {
    ...
    static Intersect(Deck1, Deck2) {
        return Deck1 & Deck2;
    }
    static Union(Deck1, Deck2) {
        return Deck1 | Deck2;
    }
    static Difference(Deck1, Deck2) {
        return Deck1 & ~Deck2;
    }
    static IsEmpty(Deck) {
        return Deck === 0;
    }
    static HighestTrumpRank(deck) {
        deck |= deck >> 8;
        deck |= deck >> 16;
        deck |= deck >> 24;
        deck &= 0x00_00_00_ff;
        if (deck & CardUtils.Valet)
            return CardUtils.Valet;
        else if (deck & CardUtils.Nine)
            return CardUtils.Nine;
    ...
    static Value(Deck, Trump) {
        let value = 0;
        for (let i = 0; i < 32; i++) {
            let currentMask = 1 << i;
            if ((Deck & currentMask) === 0)
                continue;
