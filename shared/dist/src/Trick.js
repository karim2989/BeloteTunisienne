...
export default class Trick {
    ...
    cardsIndex = 0;
    winningPlayIndex = 0;
    get WinningPlayIndex() { return this.winningPlayIndex; }
    ;
    get IsDone() { return this.cardsIndex == 4; }
    ;
    constructor(trump) {
        this.trump = trump;
        this.cards = new Int32Array(4);
    }
    tryPlayCard(card, hand) {
        /* local functions: */
        function isWrongfullyUnderRuffing(card, hand, trump, winningCard) {
        ...
        if (CardUtils.Suit(this.cards[0]) == CardUtils.Suit(card) &&
            !(CardUtils.Suit(this.cards[0]) == this.trump &&
                isWrongfullyUnderRuffing(card, hand, this.trump, this.cards[this.winningPlayIndex]))) // played leading suit => always legal (except if dodging an overruff)
         {
